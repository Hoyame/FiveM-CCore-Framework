...
AddEventHandler("charselect:animation", function(status)
    Citizen.CreateThread(function()
        ...
        enable = true
        --PedAdd(Model, UUID, Face, Outfit, Tattoo, RoleplayTable)
        --Session()
        CreateCam()
        TriggerEvent("charselect:createsign") 
        ...
        if status == true then
            while enable == true do
                Citizen.Wait(1)
                ShowHelpNotification("Press ~INPUT_FRONTEND_ACCEPT~ to select character.")
        ...
        if status == false then
            while enable == true do
                Citizen.Wait(1)
                ShowHelpNotification("Press ~INPUT_FRONTEND_ACCEPT~ to go to the character creation.")
                RequestAnimDict("mp_character_creation@customise@male_a")
                TaskPlayAnim(PlayerPedId(), "mp_character_creation@customise@male_a", "loop", 8.0, -8.0, -1, 0, 1, 0, 0, 0)
                if IsControlJustReleased(1, 201) then
                    CreateChar()
...
AddEventHandler("charselect:createsign", function(ped, name, job, money) 
    SignProp1 = CreateObject(signmodel, 1, 1, 1, false, true, false)
    SignProp2 = CreateObject(textmodel, 1, 1, 1, false, true, false)

    AttachEntityToEntity(SignProp1, ped, GetPedBoneIndex(PlayerPedId(), 58868), 0.12, 0.24, 0.0, 5.0, 0.0, 70.0, true, true, false, false, 2, true);
    AttachEntityToEntity(SignProp2, ped, GetPedBoneIndex(PlayerPedId(), 58868), 0.12, 0.24, 0.0, 5.0, 0.0, 70.0, true, true, false, false, 2, true);
