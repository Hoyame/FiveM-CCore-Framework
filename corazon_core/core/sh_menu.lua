...
function PMenu:resetMenu()
	...
	self.Base = {
		...
		Title = GM and GM.user and GM.user.name or "Menu",
...
function PMenu:ProcessControl()
	...
	local boolUP, boolDOWN, boolRIGHT, boolLEFT = IsControlPressed(1, Keys["UP"]), IsControlPressed(1, Keys["BAS"]), IsControlPressed(1, Keys["RIGHT"]), IsControlPressed(1, Keys["LEFT"])
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	local stringPrefix = (((button.r and (((GM and (GM.jobRank < button.r or GM.copRank < button.r)) or (button.rfunc and not button.rfunc())) and "~r~" or "")) or "") .. (self.Events["setPrefix"] and self.Events["setPrefix"](button, self.Data) or "")) or ""
